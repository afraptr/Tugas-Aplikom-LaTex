...
\end{euleroutput}
\begin{eulerprompt}
>(log(1)+sin(270°))^2*(27/sqrt(81)+2^2)
\end{eulerprompt}
\begin{euleroutput}
  7
\end{euleroutput}
\begin{eulerprompt}
>gcd(14,6)*length(primes(10))/2^2
\end{eulerprompt}
\begin{euleroutput}
  2
\end{euleroutput}
...
\eulersubheading{}
\begin{eulercomment}
1)Seorang petugas memeriksa suatu contoh acak tujuh kaleng ikan tuna
untuk di periksa presentase ketidakmurniannya. Data yang diperoleh
adalah 1.8, 2.1, 1.7, 1.6, 0.9, 2.7 (dalam persen). Hitunglah nilai
tengah nya
\end{eulercomment}
\begin{eulerprompt}
>A=[1.8,2.1,1.7,1.6,0.9,2.7]; mean(A)
\end{eulerprompt}
\begin{euleroutput}
  1.8
\end{euleroutput}
\begin{eulercomment}
2)Dari 5 orang mahasiswa di pilih 3 orang untuk mengikuti olimpiade
matematika. Ada berapa cara memilih peserta olimpiade tersebut?
\end{eulercomment}
\begin{eulerprompt}
>::: binomial(5,3); // nilai C(5,3)
\end{eulerprompt}
\begin{euleroutput}
...
\end{euleroutput}
\begin{eulercomment}
3) Tentukan turunan dan integral dari fungsi berikut
\end{eulercomment}
\begin{eulerprompt}
>f &= 2*x^2 + 5*x - 7
\end{eulerprompt}
